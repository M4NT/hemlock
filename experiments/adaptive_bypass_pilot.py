...
SPECS = [
    # citation_forgery — verifiable facts, expected more robust
    ...
    # jailbreak_via_context — framing-based, expected trivially reformulable
...
def run_pilot(
    ...
    for category, variant, attack_cls, variant_arg, ingest_gs, retrieval_gs in SPECS:
        # Re-instantiate guards per spec to avoid shared state
        ingest_gs_fresh = [g.__class__() for g in ingest_gs]
        retrieval_gs_fresh = [g.__class__() for g in retrieval_gs]
        ...
        for run_id in range(reps):
            ...
            trial = _run_trial(
                ...
                ingest_guards=[g.__class__() for g in ingest_gs],
                retrieval_guards=[g.__class__() for g in retrieval_gs],
                run_id=run_id,
                budget=budget,
...
def _print_summary(path: str) -> None:
    ...
    for t in trials:
        by_cat[t.attack_category].append(t)

    print("\n── Summary (metric: ingest guard bypass) ────────────────────")
    print(f"{'Category':<25} {'Bypass%':>8} {'Avg variants':>13} {'Original%':>10}")
    print("-" * 60)
    for cat, ts in sorted(by_cat.items()):
        ...
        print(f"{cat:<25} {bypass_rate*100:>7.0f}% {avg_variants:>13.1f} {orig_rate*100:>9.0f}%")
